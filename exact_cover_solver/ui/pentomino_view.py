...
def create_pentomino_view(size, grid, prev, next):
    if not size:
        return [
            ...
            [sg.Text("Program will generate all the solutions for selected "
                          "size and let's you browse solutions one by one. This is a "
                          "blocking operation that might take a while to perform.",
                          auto_size_text=True, size=(50, 5))],
            [sg.Button("3x20"), sg.Button("4x15"), sg.Button("5x12"), sg.Button("6x10")]
        ]
...
def create_grid(grid):
    return [[sg.Canvas(background_color=pentomino_colors[grid[row][col]], size=(20, 20),
                       border_width=1) for col in range(len(grid[0]))] for row in
            range(len(grid))]
